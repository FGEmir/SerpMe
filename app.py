...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        ...
        self.send_header("Content-Security-Policy", "default-src 'self'; base-uri 'self'; form-action 'self'; object-src 'none'; frame-ancestors 'none'; style-src 'self' https://fonts.googleapis.com; font-src https://fonts.gstatic.com; script-src 'self'; connect-src 'self' https://neunsivpakcbgvfxvprx.supabase.co; img-src 'self' data:; frame-src https://www.google.com https://maps.google.com")
        super().end_headers()
